...
class CustomDataGen(tf.keras.utils.Sequence):
    ...
    def __init__(self, seq_type, root_dir, data_list, cache_path=None, step_size=10, window_size=200, batch_size = 128,
    ...
    def __getitem__(self, item):
        ...
        feat_list = []
        target_list = []
        # for i in range(item, (item + self.batch_size)):
        for i in range(item*self.batch_size, (item*self.batch_size)+self.batch_size):
            seq_id, frame_id = self.index_map[i][0], self.index_map[i][1]
        ...
        if ~(self.shuffle):
            self.all_targets.append(targ.astype(np.float32))
        return feat.astype(np.float32), targ.astype(np.float32)
    
    

    
    def __len__(self):
         return int(len(self.index_map) //  self.batch_size )
